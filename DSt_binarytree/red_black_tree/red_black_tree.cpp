...
protected:
    ...
    // Helper functions, regard nullptrs as black
    Color nodePcol( TreeNode* x ) {
        if ( x->parent == nullptr ) {
            return black;
        } else {
            return (x->parent)->color;
        }
    };
    Color nodePPcol( TreeNode* x ) {
        if ( x->parent == nullptr ) {
            return black;
        } else if ( (x->parent)->parent == nullptr ){
            return black;
        } else {
            return ((x->parent)->parent)->color;
        }
    };
    ...
    void redBlackFixup( NODEPTR& root, NODEPTR& x ) {

        // Regard nullptrs as black

        // Initial check

            NODEPTR y;

            while ( nodePcol(x) == red ) {
...
void RBTree::insert( int x ) {
    ...
    redBlackFixup( root, p );

}

// NODEPTR RBTree::search( int x ) {}
