...
protected:
    ...
    int balanceFactor( NODEPTR x ) {
        ...
        int hL = subTreeHeight( x->left );
        int hR = subTreeHeight( x->right );
    ...
    void AVL_insertFixup( NODEPTR& root, NODEPTR& x ) {
        ...
        while ( z != nullptr ) {
            ...
            if ( x == z->right ) {
                ...
                {
                    if ( balanceFactor(x) < 0 ) { // right child of z is left heavy
                        rightRotate( root, x);
                        leftRotate(  root, z);
                    } else {
                        leftRotate(  root, z);
...
void AVLTree::insert( int x ) {
    ...
    AVL_insertFixup( root, p );

}

// NODEPTR AVLTree::search( int x ) {}
